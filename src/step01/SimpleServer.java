	...
	public boolean analizeCommandline(String[] args){
		if(args.length > 0){
			try{
				...
				System.out.println("Server> 入力は無視します。<analizeCommandline>");
			}
		}
		
		System.out.println("Server> ポート番号は"+port+"とします。<analizeCommandline>");
		return true;
	}
 ...
 */
	public boolean setSocket(){
		try{
			...
			System.out.println("Server> サーバソケットの生成に成功しました。<setSocket>");
			return true;
		}
		catch(Exception e){ //IOException: ソケットの生成に失敗
